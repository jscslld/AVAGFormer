...
def save_mask(pred_masks, save_lmdb_path, video_name_list, type="func"):
    # pred_masks: [bs*5, 1, 224, 224]
    pred_masks = pred_masks.squeeze(2)  # [bs*5, 224, 224]
    pred_masks = (torch.sigmoid(pred_masks) > 0.5).int()
    pred_masks = pred_masks.view(-1, 1, pred_masks.shape[-2], pred_masks.shape[-1])
    pred_masks = pred_masks.cpu().numpy().astype(np.uint8) * 255  # [N, 1, H, W]

    env = lmdb.open(save_lmdb_path, map_size=1024**4)  # 1TB max, adjust as needed

    with env.begin(write=True) as txn:
        for idx, (mask, video_name) in tqdm(enumerate(zip(pred_masks, video_name_list)), total=len(pred_masks)):
            # 将 mask 转换为 PNG bytes
            pil_img = Image.fromarray(mask[0])  # shape [1, H, W] -> [H, W]
            buffer = io.BytesIO()
            pil_img.save(buffer, format="PNG")
            img_bytes = buffer.getvalue()

            # 使用唯一键保存，例如 "video_name_0", "video_name_1" ...
            key = f"{type}_{video_name}".encode("ascii")
            txn.put(key, img_bytes)


# def save_mask(pred_masks, save_base_path, video_name_list,type="func"):
