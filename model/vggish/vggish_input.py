...
def pad_audio(data, sample_rate, target_duration=1.0):
    """Pad or repeat audio to a target duration.

    Args:
        data: np.array, mono audio waveform.
        sample_rate: int, sample rate of the audio.
        target_duration: float, target duration in seconds.

    Returns:
        np.array, audio waveform of at least `target_duration` seconds.
    """
...
def waveform_to_examples(data, sample_rate, return_tensor=True):
  ...
  """
    ...
    # Check if audio is shorter than 1 second and pad or repeat.
    target_length = int(vggish_params.SAMPLE_RATE)
    if len(data) < target_length:
        data = pad_audio(data, vggish_params.SAMPLE_RATE, 1)
    print(data.shape)
