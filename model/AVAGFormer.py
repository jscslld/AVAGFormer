...
class AVAGFormer(nn.Module):
    def __init__(self,
        ...
        self.audio_proj1 = nn.Linear(embed_dim * 20, embed_dim)
    ...
    def forward(self, audio, frames, vid_temporal_mask_flag=None):
        ...
        T = 20
        audio_feat = rearrange(audio_feat, '(bs T) K dim ->bs T K dim',T=T).squeeze(2)
        # audio_feat = self.audio_proj1(rearrange(audio_feat, '(bs T) K dim ->bs K (T dim)',T=T))
        img_feat = self.extract_feat(frames)
        final_func_mask, final_dep_mask, pred_func, pred_dep = self.head(img_feat, audio_feat,frames)
